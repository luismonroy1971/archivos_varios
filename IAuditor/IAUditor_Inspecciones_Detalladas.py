...
with ThreadPoolExecutor(max_workers=10) as executor:
    ...
    # Mostrar progreso de obtención de detalles
    for future in tqdm(as_completed(detail_futures), total=len(detail_futures), desc="Obteniendo detalles de inspecciones"):
        detail = future.result()
        if detail:
            inspection_details.append(detail)
    
    # Mostrar progreso de obtención de respuestas
    for future in tqdm(as_completed(answer_futures), total=len(answer_futures), desc="Obteniendo respuestas de inspecciones"):
